...
df_pool_len = pd.read_excel('../data/liar/processed/pool_data.xlsx')
...
class Config(object):
    ...
    def __init__(self, device):
        ...
        self._init_event_info_from_excel('../data/liar/processed/train_data.xlsx')
...
def load_data(flag, models, data_index, fine_num=0):
    ...
    if len(data_index) == 0:
        if flag == "train1":
            data_path = '../data/liar/processed/source_data.pkl'
            f = open(data_path, 'rb')
            w = pickle.load(f)
        elif flag == "train2":
            data_path = '../data/liar/processed/source_extend_data.pkl'
            f = open(data_path, 'rb')
            w = pickle.load(f)

        elif flag == "test":
            data_path = '../data/liar/processed/destination_data.pkl'
            f = open(data_path, 'rb')
            w = pickle.load(f)
        elif flag == "validate":
            data_path = '../data/liar/processed/validate_data.pkl'
            f = open(data_path, 'rb')
            w = pickle.load(f)
        elif flag == "pool":
            data_path = '../data/liar/processed/source_pool_data.pkl'
            f = open(data_path, 'rb')
            w = pickle.load(f)
    # 对于之后的任意次：2，3，4，5次
    else:
        ...
        if flag == "fine_train":
            ...
            df = pd.read_excel('../data/liar/processed/pool_data.xlsx')  # 即完整的目标域数据  加载出test的字典张量数据
            ...
            df_source_extend = pd.read_excel('../data/liar/processed/train_data.xlsx')  # 源 + 0.1目标
            ...
            df_new_train.to_excel('../data/liar/processed/new_train_data.xlsx',
            ...
            df_pool = pd.read_excel('../data/liar/processed/pool_data.xlsx')
            ...
            df_new_pool.to_excel('../data/liar/processed/new_pool_data.xlsx', index=False)  # 这是删去0.05的目标
            ...
            df1 = pd.read_excel('../data/liar/processed/new_pool_data.xlsx')
            ...
            df_train_before = pd.read_excel('../data/liar/processed/new_train_data.xlsx')
            df_train_after = pd.concat([df_train_before, df_train_after], axis=0)
            df_train_after = df_train_after.reset_index(drop=True)
            print("new_train_data文件的长度：", len(df_train_after))
            df_train_after.to_excel('../data/liar/processed/new_train_data.xlsx', index=False)
            # df_train_after.to_excel('../data/liar/processed/new_train_after_data.xlsx', index=False)  # 这里现在是 0.1的目标+之后选择出来的0.05的目标
            ...
            df2 = pd.read_excel('../data/liar/processed/new_pool_data.xlsx')
            ...
            df_pool_after.to_excel('../data/liar/processed/new_pool_data.xlsx',
                                   index=False)  # 这里现在是 0.1的目标+之后选择出来的0.05的目标
...
#                 w = pickle.load(f)
#         elif flag == "test":
#             data_path = '../data/liar/processed/sampled_data/event_label_4_.pkl'
#             with open(data_path, 'rb') as f:
#                 w = pickle.load(f)
#         elif flag == "validate":
#             data_path = '../data/liar/processed/sampled_data/event_label_4_validate_.pkl'
#             with open(data_path, 'rb') as f:
#                 w = pickle.load(f)
#         elif flag == "pool":
#             data_path = '../data/liar/processed/source_pool_data.pkl'
...
#         # 处理fine_train情况
#         if flag == "fine_train":
#             df = pd.read_excel('../data/liar/processed/sampled_data/pool_data_4.xlsx')  # 加载目标域数据
#             print("pool_data文件的长度：", len(df))
...
#
#             df_source_extend = pd.read_excel('../data/liar/processed/train_data.xlsx')
...
#             df_new_train.to_excel('../data/liar/processed/new_train_data.xlsx', index=False)
#             print("new_train_data文件的长度：", len(df_new_train))
...
#
#         elif flag == "fine_pool":
#             df_pool = pd.read_excel('../data/liar/processed/sampled_data/pool_data_4.xlsx')
#             df_new_pool = df_pool.drop(df_pool.index[data_index])  # 从池数据中去除选择的数据
#             df_new_pool = df_new_pool.reset_index(drop=True)
#             df_new_pool.to_excel('../data/liar/processed/new_pool_data.xlsx', index=False)
...
#             df_pool_after = df_pool_after.reset_index(drop=True)
#             df_pool_after.to_excel('../data/liar/processed/new_pool_data.xlsx', index=False)
#             index_pool = list(df_pool_after.index)
#             new_pool_after_data = mydict.to_bert_input_new(df_pool_after, index_pool)
#             w = new_pool_after_data
#
...
def load_dataset(data_index, flag, fine_num=0, models=""):
    '''
    文件说明：
    '../data/liar/processed/train_data.xlsx'
    '../data/liar/processed/train2_data.xlsx'
    '../data/liar/processed/test_data.xlsx'      这三个文件都是预处理数据的，即不能删改的

    '../data/liar/processed/new_train_data.xlsx'
    '../data/liar/processed/new_test_data.xlsx'  这两个文件都是微调时候，动态产生的文件，可以调整
    '''
...
def tst(models, config, train_dataset, test_loader, best_dir, flag, owner="the model"):
    ...
    if exists(best_dir):
        ...
        load_state_dict_flexible(models, torch.load(best_dir, map_location=device))
        print('加载已保存模型！')
    ...
    # ✅ 统一用 list 收集，最后转 numpy，彻底避免 concatenate 维度问题
    test_score_list = []   # 存每个样本的 [p0, p1]
    test_pred_list = []    # 存每个样本预测标签 0/1
    test_true_list = []    # 存每个样本真实标签 0/1
    ...
    with torch.no_grad():
        for i, (test_text, test_mask, test_affection, test_labels, event_labels, test_marked_label, test_data_index) in enumerate(test_loader):
            x1 = test_text.to(device)
            x2 = test_mask.to(device)
            x3 = test_affection.to(device)

            # forward=0 调用，期望返回 (score, domain_logits, marked_prob)
            predict_label, predict_domain, predict_marked_label2 = models(x1, x2, x1, x2, x3, x3, flag)

            # ✅ predict_label 统一成 [B,2] 的 tensor（避免 batch=1 时 squeeze 变 1D）
            if predict_label.dim() == 1:
                predict_label = predict_label.view(1, -1)

            # ✅ 拉到 CPU，numpy 化
            predict_label_np = to_np(predict_label)   # shape: (B,2) 或 (B, C)
            # 如果你的 class_classifier 永远是2类，这里就是 (B,2)
            if predict_label_np.ndim == 1:
                # 兜底：万一还是 1D，就当成单样本
                predict_label_np = predict_label_np.reshape(1, -1)

            # ✅ 预测类别（基于最大概率）
            test_argmax = np.argmax(predict_label_np, axis=1)  # shape: (B,)
            ...
            # ✅ 追加到 list
            test_score_list.append(predict_label_np)       # (B,2)
            test_pred_list.append(test_argmax.reshape(-1)) # (B,)
            test_true_list.append(y_true)                  # (B,)

            # 你原来的 total_len 逻辑（不影响指标）
            total_len += len(y_true)

    # ✅ 循环结束后拼起来：score 是 [N,2]，pred/true 是 [N]
    test_score = np.concatenate(test_score_list, axis=0)  # shape: (N,2)
    test_pred = np.concatenate(test_pred_list, axis=0)    # shape: (N,)
    test_true = np.concatenate(test_true_list, axis=0)    # shape: (N,)
    ...
    # ✅ 直接取第2列作为正类概率（N,)
    # 若未来你改成 logits 输出，这里需要先 softmax 再取 [:,1]
    ...
    test_precision2, test_recall2, test_f12, _ = precision_recall_fscore_support(
        test_true, test_pred, average="micro"
    )

    print("Classification Acc: %.4f, AUC-ROC: %.4f" % (test_accuracy, test_aucroc))
...
def gogogo(config, models):
    ...
    def _balanced_ratio_split(train_ds, frac=0.2, seed=42, label_index=4, verbose=True):
        ...
        # 1) 按 event_label 分组（用索引，不存 sample，省内存&更稳定）
        ...
        # 3) 若类别容量不足导致预算没用满，则把剩余预算分配给还有余量的类别
        allocated = sum(target.values())
        remaining = desired_total - allocated

        if remaining > 0:
            # 还有“可增加”的类别
            caps = [lab for lab in labels if target[lab] < len(groups[lab])]
            ptr = 0
            while remaining > 0 and len(caps) > 0:
                lab = caps[ptr % len(caps)]
                if target[lab] < len(groups[lab]):
                    target[lab] += 1
                    remaining -= 1
                # 更新 caps（可能某类满了）
                caps = [x for x in caps if target[x] < len(groups[x])]
                ptr += 1

        # 4) 按 target[lab] 随机取样，构造 less / left 索引
        less_indices, left_indices = [], []
        stats = {}

        for lab in labels:
            idxs = groups[lab]
            g = torch.Generator().manual_seed(seed + int(lab) * 17)

            perm = torch.randperm(len(idxs), generator=g).tolist()
            pick_n = int(target[lab])

            picked = [idxs[p] for p in perm[:pick_n]]
            rest = [idxs[p] for p in perm[pick_n:]]
    ...
    add_test_dataset, _discarded_train_dataset = random_split(train_dataset, [add_test_size, test_size])
    ...
    # ===== ✅ 关键改动：按比例抽样 + 类别均衡（不再“每类0.2抽样”）=====
    less_frac = float(getattr(config, "less_frac", 0.6))
    print(f"[CONFIG] gogogo less_frac = {less_frac}")

    less_train_dataset, left_dataset, sample_counts = _balanced_ratio_split(
        train_dataset,
        frac=less_frac,
        seed=42,
        label_index=4,
    ...
    train_size = int(1.0 * len(pool_dataset))
    test_size = len(pool_dataset) - train_size
    pool_dataset, _discarded_pool_dataset = random_split(pool_dataset, [train_size, test_size])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Hugging Face model id or local path for the causal LLM. "
             "May also be set with SHARP_LLM_MODEL.",
    )
    parser.add_argument(
        "--prompt-seed",
        default=os.environ.get(
            "SHARP_PROMPT_SEED",
            str(REPO_ROOT / "data" / "liar" / "prompt_seed" / "clippool_16_release.xlsx"),
        ),
        help="Dataset-specific XLSX seed used to pre-train the soft prompt. "
             "May also be set with SHARP_PROMPT_SEED.",
    )
    ...
    if not os.path.isfile(args.prompt_seed):
        parser.error(f"LIAR prompt seed not found: {args.prompt_seed}")
