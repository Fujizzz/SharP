...
#         统一为小写
#     '''
#     words_lists = [x.casefold() for x in token_words]
#     return words_lists
#
...
#
#     model = word2vec.Word2Vec(sentences, vector_size=200, window=7, min_count=1, sg=1, negative=20)  # 训练skip-gram模型; 默认window=5
#     model.save('W2V.model')
#
# def loadw2c():
#     model = Word2Vec.load('W2V.model')  # 加载模型
#     try:
#
...
def add_event_label(filepath_list):
    # 初始化的时候给全部文件（filepath_list）添加一列事件标签列
    for i, filepath in enumerate(filepath_list):
        insert_list = []
        df = pd.read_csv(filepath, encoding='unicode_escape')
        for index, row in df.iterrows():  # 遍历数据表 , 插入类别标签i
            insert_list.append(i)
        df['event_label'] = insert_list   # 末尾列插入的是行数
        df.to_csv(filepath, index=False)

def add_if_marked_label(filepath_list,mark):
    # 1是指这条新闻被标注过，0是没有被标注过
    for i, filepath in enumerate(filepath_list):
        insert_list = []
        df = pd.read_csv(filepath, encoding='unicode_escape')
        for index, row in df.iterrows():  # 遍历数据表 , 插入类别标签mask
...
def read_data(filepath):
    ...
    # 删除label中 不等于 0/1的行
    # d既是要删除的数据
    d = df[(df['label'] != '0') & (df['label'] != '1') & (df['label'] != '1.0') & (df['label'] != '0.0')]
...
class LoadSingleSentenceClassificationDataset:
    def __init__(self,
                 vocab_path = "./vocab.txt",         # Bert词汇表的文件
                 tokenizer = None,
                 batch_size=64,
    ...
    def data_process(self, filepath_list, flag):
        ...
        # 4个源域数据 + 目标域数据的0.1
            ...
            df_train.to_excel('../../data/pheme/processed/train_data.xlsx', index=False)
            print(df_train.tail())
            return df_train

        # 得到5个文件里面的全部数据,但其中，相当于就是把test和train合并起来
        ...
        #         else:
        #             df_tmp = read_data(filepath)
        #             df_train2 = pd.concat([df_train2, df_tmp], axis=0)
        #     print("all文件共", len(df_train2))
        #     df_train2 = df_train2.reset_index(drop=True)
        #     print("train2", df_train2.head())
        # 用于鉴别是否已打标签的， 这里相当于应该只拼接 训练集df_train, 和 候选集 df_pool
        elif flag == "train2":
            df_train = pd.read_excel('../../data/pheme/processed/train_data.xlsx')
            df_test = pd.read_excel('../../data/pheme/processed/test_data.xlsx')
            df_pool = pd.read_excel('../../data/pheme/processed/events/event_2_pool.xlsx')
            df_train2 = pd.concat([df_train, df_pool], axis=0)
            df_train2 = df_train2.reset_index(drop=True)
            print("train2文件最后有", len(df_train2))
            df_train2.to_excel('../../data/pheme/processed/train2_data.xlsx', index=False)
            return df_train2
        elif flag == "pool":
            ...
            for i, filepath in enumerate(filepath_list):
                ...
                # 这个df_test是全局的，在某处得到赋值
                # 不会因为df_test与df_pool有不同元组而在df_pool留下杂乱的新信息
                df_pool = pd.concat([df_pool, df_test, df_test]).drop_duplicates(keep=False)  # 这一行相当于从目标域数据集中删去固定的0.25的数据
                df_ter = df_pool.sample(frac=0.25, random_state=1234, axis=0)                  # 原来是0.125
                ...
                df_pool.to_excel('../../data/pheme/processed/events/event_2_pool.xlsx', index=False)
                return df_pool
        elif flag == "test":
            df_test = []
        # 从目标域中拿到0.2的初始数据集
            ...
            df_test = df_test.sample(frac=0.2, random_state=1234, axis=0)                  # 这里先抽出来0.2的样本作为不动的测试集
            print("test文件最后有", len(df_test))                                            # 再把样本的标记值给改掉
            df_test = df_test.reset_index(drop=True)
            df_test.to_excel('../../data/pheme/processed/test_data.xlsx', index=False)
            return df_test
    ...
    def to_bert_input(self, df, df_index, flag):
        ...
        for i in range(0,len(df)):
            ...
            s, l, e, il = df.iloc[i,2], df.iloc[i,1], df.iloc[i,4],df.iloc[i,3]
        ...
        if flag == "source":
            df = pd.read_excel('../../data/pheme/processed/train_data.xlsx')
            df.insert(loc=2, column='affection', value=affection)
            df.to_excel('../../data/pheme/processed/train_data.xlsx', index=False)
        elif flag == "source_extend":
            df = pd.read_excel('../../data/pheme/processed/train2_data.xlsx')
            df.insert(loc=2, column='affection', value=affection)
            df.to_excel('../../data/pheme/processed/train2_data.xlsx', index=False)
        elif flag == "source_pool":
            df = pd.read_excel('../../data/pheme/processed/events/event_2_pool.xlsx')
            df.insert(loc=2, column='affection', value=affection)
            df.to_excel('../../data/pheme/processed/events/event_2_pool.xlsx', index=False)
        elif flag == "destination":
            df = pd.read_excel('../../data/pheme/processed/test_data.xlsx')
            df.insert(loc=2, column='affection', value=affection)
            df.to_excel('../../data/pheme/processed/test_data.xlsx', index=False)
    ...
    def to_bert_input_new(self, df, df_index):
        ...
        for i in range(0,len(df)):
            # 拿到文本数据 和 标签 和 事件类别标签 和 是否有标注的标签
            s, l, e, il = df.iloc[i,3], df.iloc[i,1], df.iloc[i,5],df.iloc[i,4]
            # print(s,type(s))
...
if __name__ == '__main__':
    ...
    df_ter = 0.1                          # df_ter 是在构建训练数据时，从目标域中抽取的数据
    df_test = 0
    # 先构建测试集数据
    ...
    # 把数据打包进入pkl文件里
    with open('../../data/pheme/processed/source_data.pkl', 'wb') as f:
        pickle.dump(source_data, f)
    with open('../../data/pheme/processed/source_pool_data.pkl', 'wb') as f:
        pickle.dump(source_pool_data, f)
    with open('../../data/pheme/processed/source_extend_data.pkl', 'wb') as f:
        pickle.dump(source_extend_data, f)
    with open('../../data/pheme/processed/destination_data.pkl', 'wb') as f:
